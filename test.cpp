...
Move getStruggle() {
    string moveName = "struggle", moveType = "typeless", moveCategory = "physical", s1 = "none", s2 = "none";
...
public:
    ...
    Move& getRandomMove() {
        if (moveSet.empty()) {
            moveSet.push_back(getStruggle());
            return moveSet[0];
        }
        if (isOutOfUsableMoves()) {
            return moveSet[0];
        }
...
public:
    ...
    void executeTurn(Pokemon &user_pokemon, Move &user_move, Pokemon &opponent_pokemon, Move &opponent_move) {
        ...
        if (firstAttacker->current_health <= 0) return;
...
Pokemon readPokemonData(int pokedexNumber) {
    ...
    while (currentCounter != pokedexNumber && getline(pokedex_file, pokedex_line) && getline(pokemon_moves_file, moveset_line)) {
        currentCounter++;
    }

    pokedex_file.close();
    pokemon_moves_file.close();
...
vector <float> getEnvironmentState(vector <Pokemon> &agent_party, int agent_idx, vector <Pokemon> &opp_party, int opp_idx) {
    vector <float> state;

    Pokemon &agent_active = agent_party[agent_idx];
    Pokemon &opp_active = opp_party[opp_idx];

    state.push_back(static_cast<float>(agent_active.current_health)/agent_active.hp);
    
    state.push_back(agent_active.attStage / 6.0f);
    state.push_back(agent_active.defStage / 6.0f);
    state.push_back(agent_active.spAStage / 6.0f);
    state.push_back(agent_active.spDStage / 6.0f);
    state.push_back(agent_active.speStage / 6.0f);

    for (int i = 0; i < 6; ++i) {
        state.push_back(static_cast<int>(agent_active.currentStatus) == i ? 1.0f : 0.0f);
    }

    state.push_back(static_cast<float> (opp_active.current_health)/opp_active.hp);

    state.push_back(opp_active.attStage / 6.0f);
    state.push_back(opp_active.defStage / 6.0f);
    state.push_back(opp_active.spAStage / 6.0f);
    state.push_back(opp_active.spDStage / 6.0f);
    state.push_back(opp_active.speStage / 6.0f);

    for (int i=0; i<6; i++) {
        state.push_back(static_cast<int>(opp_active.currentStatus) == i?1.0f:0.0f);
    }

    for (int i=0; i<4; i++) {
        if (i<agent_active.moveSet.size()) {
            Move &m = agent_active.moveSet[i];
            state.push_back(m.power/150.0f);
            state.push_back(m.accuracy/100.0f);
            state.push_back(m.effectiveness(opp_active));
            state.push_back(m.isStab(agent_active));
        }
        else {
    ...
    for (int i=0; i<agent_party.size(); i++) {
        if (i!=agent_idx) {
            state.push_back(static_cast<float> (agent_party[i].current_health)/agent_party[i].hp);
        }
    }
...
bool isValidAction(vector <Pokemon> &party, int idx, int action) {
    if (action>=0 && action<4) {
        if (action>=party[idx].moveSet.size()) return false;
        if (party[idx].moveSet[action].pp<=0 && !party[idx].isOutOfUsableMoves()) return false;
        return true;
    }
    else if (action>=4 && action<10) {
        int switchIdx = action-4;
        if (switchIdx>=party.size() || switchIdx == idx || party[switchIdx].isFainted()) return false;
        return true;
    }
    return false;
}

float evaluateState(vector <Pokemon> &myParty, vector <Pokemon> &oppParty) {
    float myHP = 0.0f;
    for (auto &p : myParty) myHP += p.current_health;
    float oppHP = 0.0f;
    for (auto &p : oppParty) oppHP += p.current_health;
    return myHP - oppHP;
}
...
public:
    Battle battle;
    vector <Pokemon> agent_party;
    vector <Pokemon> opp_party;
    int agent_idx;
    int opp_idx;

    RLEnvironment(vector <Pokemon> agent, vector <Pokemon> opp) {
        agent_party = agent;
        opp_party = opp;
        agent_idx = 0;
        opp_idx = 0;
    ...
    vector <float> reset () {
        agent_idx = 0;
        opp_idx = 0;
        return getEnvironmentState(agent_party, agent_idx, opp_party, opp_idx);
    }

    void render() {
        cout << "\n=========================================\n";
        cout << "[AI] ";
        agent_party[agent_idx].drawHealthBar();
        cout << "                V/S                    \n";
        cout << "[YOU] ";
        opp_party[opp_idx].drawHealthBar();
        cout << "=========================================\n";
    }

    void showMoves() {
        cout << "Choose a move for " << opp_party[opp_idx].name << " (Enter number):\n";
        opp_party[opp_idx].displayMoves();
    }

    void showSwitches() {
        cout << "Select Pokemon to switch to (Enter number):\n";
        battle.displayParty(opp_party);
    }
    ...
    StepResult step(int action, int opp_action = -1) {
        float reward = 0.0f;

        int initialAgentHP = 0;
        for (auto &p : agent_party) initialAgentHP += p.current_health;

        int initialOpponentHP = 0;
        for (auto &p : opp_party) initialOpponentHP += p.current_health;

        Pokemon &agentPokemon = agent_party[agent_idx];
        Pokemon &oppPokemon = opp_party[opp_idx];

        Move *opp_move = nullptr;
        bool oppSwitched = false;

        if (opp_action >= 4 && opp_action < 10) {
            int switchIdx = opp_action - 4;
            if (switchIdx < opp_party.size() && switchIdx!=opp_idx && opp_party[switchIdx].current_health > 0) {
                opp_idx = switchIdx;
                cout << "You sent out " << opp_party[opp_idx].name << "!\n";
                oppSwitched = true;
            }
        }

        Pokemon &currentOppPokemon = opp_party[opp_idx];

        if (!oppSwitched) {
            if (opp_action >= 0 && opp_action < (int)currentOppPokemon.moveSet.size()) {
                opp_move = &currentOppPokemon.moveSet[opp_action];
            }
            else {
                opp_move = &currentOppPokemon.getRandomMove();
            }
        }

        if (action >= 0 && action < 4) {
            if (action < agentPokemon.moveSet.size()) {
                Move &agent_move = agentPokemon.moveSet[action];
                if (agent_move.pp <= 0 && !agent_party[agent_idx].isOutOfUsableMoves()) {
                    reward -= 10.0f;
                    if (!oppSwitched) battle.turnManager.executeAttack(currentOppPokemon, *opp_move, agentPokemon);
                } else {
                    if (oppSwitched) {
                        battle.turnManager.executeAttack(agent_party[agent_idx], agent_move, currentOppPokemon);
                    } else {
                        battle.turnManager.executeTurn(agent_party[agent_idx], agent_move, currentOppPokemon, *opp_move);
                    }
                }
            }
            else {
                reward -= 10.0f;
                if (!oppSwitched) {
                    battle.turnManager.executeAttack(currentOppPokemon, *opp_move, agentPokemon);
                }
            }
        }
        else if (action >= 4 && action < 10){
            int switchIdx = action - 4;
            if (switchIdx < agent_party.size() && switchIdx != agent_idx && agent_party[switchIdx].current_health > 0) {
                agent_idx = switchIdx;
                cout << "AI sent out " << agent_party[agent_idx].name << "!\n";
                if (!oppSwitched) battle.turnManager.executeAttack(currentOppPokemon, *opp_move, agent_party[agent_idx]);
            }
            else {
                reward -= 10.0f;
                if (!oppSwitched) battle.turnManager.executeAttack(currentOppPokemon, *opp_move, agent_party[agent_idx]);
        ...
        if (oppSwitched || action >= 4 || action >= agentPokemon.moveSet.size() || (action < agentPokemon.moveSet.size() && agentPokemon.moveSet[action].pp <= 0 && !agent_party[agent_idx].isOutOfUsableMoves())) {
            battle.turnManager.applyEndOfTurnStatus(agent_party[agent_idx]);
            battle.turnManager.applyEndOfTurnStatus(opp_party[opp_idx]);
        }

        if (opp_party[opp_idx].current_health <= 0) {
            reward += 50.0f;
            for (int i=0; i<opp_party.size(); i++) {
                if (opp_party[i].current_health > 0) {
                    opp_idx = i;
                    cout << "You sent out " << opp_party[opp_idx].name << "!\n";
                    break;
                }
        ...
        if (agent_party[agent_idx].current_health <= 0) {
            reward -= 50.0f;
            for (int i=0; i<agent_party.size(); i++) {
                if (agent_party[i].current_health > 0) {
                    agent_idx = i;
                    cout << "AI sent out " << agent_party[agent_idx].name << "!\n";
                    break;
                }
        ...
        int finalAgentHP = 0;
        for (auto &p : agent_party) finalAgentHP += p.current_health;

        int finalOppHP = 0;
        for (auto &p : opp_party) finalOppHP += p.current_health;

        reward += (initialOpponentHP - finalOppHP);
        reward -= (initialAgentHP - finalAgentHP);

        bool done = battle.isPartyDefeated(agent_party) || battle.isPartyDefeated(opp_party);

        if (done) {
            if (battle.isPartyDefeated(opp_party)) reward += 1000.0f;
            if (battle.isPartyDefeated(agent_party)) reward -= 1000.0f;
        }

        return {getEnvironmentState(agent_party, agent_idx, opp_party, opp_idx), reward, done};
    }

    void startBattleWithModel(function<int(vector<float>)> predict_fn) {
        cout << "\nBATTLE START!\n";
        cout << "Your Pokemon : ";
        battle.displayParty(opp_party);
        
        int turn_count = 0;
        while (!battle.isPartyDefeated(opp_party) && !battle.isPartyDefeated(agent_party)) {
            turn_count++;
            
            Pokemon &user_pokemon = opp_party[opp_idx];
            Pokemon &ai_pokemon = agent_party[agent_idx];

            cout << "Turn " << turn_count << "\n";
            user_pokemon.drawHealthBar();
            cout << "                V/S                    \n";
            ai_pokemon.drawHealthBar();
            ...
            vector<float> state = getEnvironmentState(agent_party, agent_idx, opp_party, opp_idx);
            int ai_action = predict_fn(state);

            Move ai_move;
            bool ai_switched = false;

            if (ai_action >= 4 && ai_action < 10) {
                int switch_target = ai_action - 4;
                if (switch_target < agent_party.size() && switch_target != agent_idx && agent_party[switch_target].current_health > 0) {
                    cout << "AI's " << agent_party[agent_idx].name << ", return. ";
                    agent_idx = switch_target;
                    cout << "AI sends out " << agent_party[agent_idx].name << "!\n";
                    ai_switched = true;
                } else {
                    ai_move = ai_pokemon.getRandomMove();
                }
            } else {
                if (ai_action >= 0 && ai_action < ai_pokemon.moveSet.size()) {
                    ai_move = ai_pokemon.moveSet[ai_action];
                    if (ai_move.pp <= 0 && !ai_pokemon.isOutOfUsableMoves()) {
                        ai_move = ai_pokemon.getRandomMove();
                    }
                } else {
                    ai_move = ai_pokemon.getRandomMove();
            ...
            if (choice == 'S' || choice == 's') {
                ...
                char switch_choice;
                cin >> switch_choice;
                while(!(switch_choice - '0' >= 1 && switch_choice - '0' <= opp_party.size()) || opp_party[(switch_choice - '0')-1].current_health<=0) {
                    if(!(switch_choice - '0' >= 1 && switch_choice - '0' <= opp_party.size())) {
                        cout << "Invalid! Enter again: ";
                        cin >> switch_choice;
                    }
                    else if (opp_party[(switch_choice-'0')-1].isFainted()){
                        cout << opp_party[(switch_choice-'0')-1].name << " has fainted! Enter again: ";
                        cin >> switch_choice;
                    }
                }
                if (switch_choice - '0' - 1 != opp_idx) {
                    cout << opp_party[opp_idx].name << ", return. ";
                    opp_idx = (switch_choice-'0')-1;
                    cout << opp_party[opp_idx].name << ", I choose you!\n";
                    user_switched = true;
                } else {
                    user_switched = true; 
                }
            ...
            if (choice == 'F' || choice == 'f') {
                ...
                while (true) {
                    if (!(user_moveID - '0' >= 1 && user_moveID - '0' <= opp_party[opp_idx].moveSet.size())) {
                        cout << "Invalid! Enter again: ";
                        cin >> user_moveID;
                    } else if (opp_party[opp_idx].moveSet[(user_moveID - '0') - 1].pp <= 0 && !opp_party[opp_idx].isOutOfUsableMoves()) {
                        cout << "That move has no PP left! Select another move: ";
                        cin >> user_moveID;
                    } else {
                        break;
                ...
                user_move = &opp_party[opp_idx].moveSet[(user_moveID - '0')-1];
            }

            if (user_switched && ai_switched) {
                battle.turnManager.applyEndOfTurnStatus(opp_party[opp_idx]);
                battle.turnManager.applyEndOfTurnStatus(agent_party[agent_idx]);
            } else if (user_switched && !ai_switched) {
                battle.turnManager.executeAttack(agent_party[agent_idx], ai_move, opp_party[opp_idx]);
                battle.turnManager.applyEndOfTurnStatus(opp_party[opp_idx]);
                battle.turnManager.applyEndOfTurnStatus(agent_party[agent_idx]);
            } else if (!user_switched && ai_switched) {
                battle.turnManager.executeAttack(opp_party[opp_idx], *user_move, agent_party[agent_idx]);
                battle.turnManager.applyEndOfTurnStatus(opp_party[opp_idx]);
                battle.turnManager.applyEndOfTurnStatus(agent_party[agent_idx]);
            } else {
                battle.turnManager.executeTurn(opp_party[opp_idx], *user_move, agent_party[agent_idx], ai_move);
            }

            if (agent_party[agent_idx].current_health <= 0) {
                cout << "AI's " << agent_party[agent_idx].name << " fainted!\n";
                for (int i = 0; i < agent_party.size(); i++) {
                    if (agent_party[i].current_health > 0) {
                        agent_idx = i;
                        cout << "AI sent out " << agent_party[agent_idx].name << "!\n";
            ...
            if (opp_party[opp_idx].current_health <= 0) {
                cout << "Your " << opp_party[opp_idx].name << " fainted!\n";
                
                if (!battle.isPartyDefeated(opp_party)) {
                    cout << "Select next Pokemon to send out:\n";
                    for (int i = 0; i < opp_party.size(); i++) {
                        opp_party[i].drawHealthBar();
                        cout << "\n";
                    }
    
                    char next_choice;
                    cin >> next_choice;
                    while(!(next_choice - '0' >= 1 && next_choice - '0' <= opp_party.size()) || opp_party[(next_choice - '0')-1].current_health==0) {
                        if(!(next_choice - '0' >= 1 && next_choice - '0' <= opp_party.size())) {
                            cout << "Invalid! Enter again: ";
                            cin >> next_choice;
                        }
                        else if (opp_party[(next_choice-'0')-1].isFainted()){
                            cout << opp_party[(next_choice-'0')-1].name << " has fainted! Enter again: ";
                            cin >> next_choice;
                        }
                    }
                    
                    opp_idx = (next_choice - '0') - 1;
                    cout << "You sent out " << opp_party[opp_idx].name << "!\n";
                }
            }
        }
        ...
        if(battle.isPartyDefeated(opp_party)) {
            cout << "You have no Pokemon left! AI WINS!\n";
        } else {
            cout << "You defeated the AI! YOU WIN!\n";
        }
        cout << "BATTLE OVER!\n";
    }
};