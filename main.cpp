...
public:
    ...
    Move getRandomMove() {
        return moveSet[getRandomInt(0, moveSet.size()-1)];
    }
...
public:
    ...
    void handleAttack(vector<Pokemon> &user_party, int &user_idx, vector<Pokemon> &opponent_party, int &opponent_idx, Move &opponent_move) {
        ...
        while(!(user_moveID - '0' >= 1 && user_moveID - '0' <=user_party[user_idx].moveSet.size())) {
            cout << "Invalid! Enter again: ";
            cin >> user_moveID;
        }
...
Pokemon readPokemonData(int pokedexNumber) {
    ...
    shufflePossibleMoves(possibleMoves);
...
int main() {
    ...
    // vector <Pokemon> pokemon_db = read_pokedex_data(pokedex_file);
    
    // for (int i=1; i<=151; i++) pokemon_db[i].display();
